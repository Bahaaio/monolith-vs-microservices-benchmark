...
# ---------------------------------------------------------------------------
MISSING=()
[[ -z "$SSH_KEY" ]]                        && { error "SSH_KEY is required. Usage: SSH_KEY=~/.ssh/my-key.pem $0"; exit 1; }
...
run_jmeter_remote() {
    ...
    local jmeter_host="$2"
    local target_ip="$3"
    local local_output="$RESULTS_DIR/${arch}_${TIMESTAMP}.jtl"
...
test_monolith_aws() {
    ...
    for i in $(seq 1 30); do
        ...
        [[ $i -eq 30 ]] && { error "JMeter installation timed out"; return 1; }
        sleep 10
    done
...
run_analysis() {
    ...
    if [[ -z "$mono_file" ]] || [[ -z "$micro_file" ]]; then
        warn "Cannot run comparison — need both result files."
        return 0
    fi
